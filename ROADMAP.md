...
## Phase 11 — Draft-Day Polish — **DONE (2026-08-12)**
...
- [x] **Draft pause/reconnect**: auto-reconnect on page load if a league was previously connected — before this, a refreshed or crashed tab required manually clicking Connect again even though the league ID was already saved. Real risk over a multi-hour live draft.
- [x] **Clear alerts / connection recovery visibility**: live picks polling now tracks and displays a "last updated Xs ago" indicator, with a visible warning if data goes stale beyond 15s — previously a failed poll only showed a small error line with no sense of how stale the data actually was. Uses an independent 5s tick so the warning still advances even through repeated identical polling failures (which wouldn't otherwise trigger a React re-render).
- [x] **Manual override**: added a "Refresh now" button next to the staleness indicator, to force an immediate poll instead of waiting up to 3s for the next automatic one — useful in the exact moment you need to know "did that just sell?"
- [x] **Performance**: memoized the economy/scarcity/bidders/dynamicValue pipeline (previously recomputed on *every* render, including every keystroke while typing in the player search box) so it only recalculates when `baseline`/`picks`/`data` actually change.
- [x] **Error handling**: connect-flow errors now give a plain-English message for the most likely real mistake (wrong league ID → "League {id} not found - double check the League ID" instead of a raw "404" URL dump); confirmed the app recovers cleanly (clears stale data, no crash) and can immediately reconnect successfully afterward.
- [x] **Basic responsive check**: added `overflow-x: auto` so wide tables scroll within themselves rather than breaking page layout on a narrow viewport. Verified no page-level or table-level horizontal overflow at ~675px width; couldn't get the test tooling to simulate a true ~375px phone width this session, so that specific size is unverified directly — the CSS fallback (scroll, not breakage) should still hold regardless, but treat this as unconfirmed rather than fully verified.
- [x] **Real bug caught during verification, not an app bug**: hit a "React hooks order changed" crash while testing in a browser tab that had survived many rounds of hot-reloading this session. Verified it was stale Vite HMR module state in that one tab — a fresh tab against a freshly restarted dev server showed no error. Documented here so it isn't mistaken for a real defect if seen again during development.
