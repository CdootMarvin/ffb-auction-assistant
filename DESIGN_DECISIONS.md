...
## 2026-08-12 — Not modeling the QB scarcity cliff yet, despite clear evidence it exists

**Decision:** confirmed via Phase 9 historical accuracy analysis (see MODELING.md Layer 1/2 and ROADMAP.md Phase 9) that the top 6 QBs in the 2025 draft sold for a mean of +$7.50 over the model's prediction (each individually $19-31 over), while the remaining 21 QBs sold for a mean of -$2.62 under prediction — a real scarcity cliff at the startable/non-startable QB boundary, not a uniform bias. Explicitly discussed building a curve-shape adjustment to capture this. Decided not to, for now.

**Why:** this is one historical draft. A cliff adjustment is structurally more specific than a simple scalar tweak (unlike the FLEX-split fix, which was validated by internal consistency logic, not curve-fitting to one draft) — it would mean fitting a discontinuity off a sample of 6 elite-QB picks from a single season. The top-6 cutoff itself was chosen by eyeballing where the split happened in this one dataset; there's no evidence it's 6 and not 5 or 8, or that last year's QB pricing wasn't itself a one-off (e.g. leaguemates reacting to a first year of 2QB scarcity) rather than a persistent pattern. Building this now would repeat exactly the overfitting risk the project has avoided everywhere else (FLEX split, keeper formula, replacement-rank conventions).

**How to apply:** revisit once the actual 2026 draft provides a second real data point for this league under the 2QB/SuperFlex format. If the cliff shows up again independently, that's real evidence of a persistent pattern worth modeling; if it doesn't, the 2025 draft was likely an outlier. Until then, current values for elite QBs should be read with this known, documented limitation in mind (likely underpriced relative to what the market will actually pay) rather than silently trusted.
